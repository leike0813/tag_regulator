# runtime-script-governance Specification
...
若脚本直接参与 skill 最终输出收敛（例如去重、稳定排序），且在 `SKILL.md` 中定义了调用时机与回退行为，则该脚本 MAY 保留在 `tag-regulator/`，并且 `SKILL.md` MUST 明确其调用顺序与失败回退。
